/**
 * PACMAN SWT - Standalone SWT Application
 *
 * GENERAL DESCRIPTION:
 * A standalone SWT conversion of the classic Pacman ImageJ plugin.
 * All ImageJ-specific dependencies (ColorProcessor, ImagePlus, Overlay) have been
 * replaced with SWT Canvas and native graphics (GC).
 *
 * Features include:
...
public class Pacman_SWT {
    ...
    // SWT Colors and Fonts (allocated once, disposed on exit)
    ...
    public static void main(String[] args) {
        new Pacman_SWT().start();
    }

    private void start() {
        display = new Display();
        shell   = new Shell(display);
        shell.setText("Pacman SWT - Level " + level);
        ...
        // Paint listener – renders one frame
        ...
        // Compute shell size to accommodate the canvas exactly
        Point preferred = shell.computeSize(W, H);
        shell.setSize(preferred);
        shell.open();
        canvas.setFocus();

        // Schedule the first game-loop tick
        scheduleGameLoop();

        // SWT event loop – exits when the shell is disposed (e.g. via ESC or window close)
    ...
    private void scheduleGameLoop() {
        display.timerExec(40, new Runnable() {
            ...
            public void run() {
                ...
                scheduleGameLoop(); // re-schedule
            }
        });
    }

    // Show a simple "game over" message in the shell title area (non-blocking)
    private void showGameOver() {
        shell.setText("GAME OVER – Final Score: " + score);
    }

    // ---------------------------------------------------------------------------
    // Level data (identical to original plugin)
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // Game logic (unchanged from original plugin)
    // ---------------------------------------------------------------------------
        ...
        for (Entity g : ghosts) {
            ...
            if (Math.abs(pac.x - g.x) < TILE * 0.7 && Math.abs(pac.y - g.y) < TILE * 0.7) {
                if (powerTimer > 0) {
                    ...
                    lives--;
                    setupLevel(false);
                    // Freeze the game loop for 600 ms (non-blocking) then resume
                    frozen = true;
    ...
    private void checkLevelComplete() {
        ...
        if (!dots) {
            level++;
            setupLevel(false);
            // Freeze the game loop for 1 s (non-blocking) then resume
            frozen = true;
    ...
    private void renderFrame(GC gc) {
        ...
        if (!gameStarted) {
            ...
            gc.drawText("PACMAN SWT", ARENA_W / 2 - 90, H / 2 - 50, true);
            ...
            // SWT fillArc: angles are in degrees, counter-clockwise from 3 o'clock
    ...
    // ---------------------------------------------------------------------------
    // ---------------------------------------------------------------------------
    // SWT Color and Font allocation / disposal
    // ---------------------------------------------------------------------------
